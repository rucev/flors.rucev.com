...
const LandingBg = ({ windowsType }: { windowsType: WindowsType }) => {
  const version = windowsType === 'sm/landscape' ? 'min' : windowsType === 'sm/portrait' ? 'reduced' : windowsType === 'md/portrait' || windowsType === 'md/landscape' ? 'reduced-md' : windowsType === 'lg/portrait' ? 'complete-v' : 'complete-h'

  return (<>
    {
      version === 'min' &&
      <div className={`absolute max-w-screen z-0 top-2 w-full right-0`}>
        <img src="/my-face.jpeg" alt="That's me!" className="absolute w-44 rounded-xl top-20 right-2/12 z-30" />
        ...
        <div className="animate-scale-1 absolute w-64 h-64  bg-base-100 rounded-full top-15 right-2/12  z-10"></div>
      </div>
    }
    {
      version === 'reduced' && <div className='absolute w-full top-50 left-0'>
        <img src="/my-face.jpeg" alt="That's me!" className="absolute w-46 rounded-xl bottom-1/7 left-3/12 top-25 z-30" />
        <div className="animate-scale-2 absolute w-80 h-80 bg-accent rounded-full top-30 left-2/12 z-10"></div>
        ...
        <div className="animate-scale-2 absolute w-32 h-32 bg-secondary rounded-full top-20 left-7/12 z-0"></div>
        <div className="animate-scale-1 absolute w-32 h-32 bg-secondary rounded-full top-70 left-1/12 z-20"></div>
      </div>
    }
    {
      version === 'reduced-md' && <div className='absolute w-full top-35 left-1/5'>
        <div className="absolute h-[70vh] w-3/5 z-0 top-3 left-0">
          <img src="/my-face.jpeg" alt="That's me!" className="absolute h-80 rounded-xl left-6/12 top-20 z-30" />
          <div className="animate-scale-2 absolute w-72 h-72 bg-accent rounded-full left-7/12 top-30 z-20"></div>
          <div className="animate-scale-1 absolute w-40 h-40 bg-secondary rounded-full left-5/12 top-10 z-10"></div>
          <div className="animate-scale-3 absolute w-80 h-80 bg-base-100 rounded-full left-7/12 top-3 z-0"></div>
          <div className="animate-scale-2 absolute w-60 h-60 bg-primary rounded-full top-20 left-1/12 z-10"></div>
          <div className="animate-scale-1 absolute w-32 h-32 bg-base-100 rounded-full top-65 left-4/12 z-10"></div>
          <div className="animate-scale-3 absolute w-56 h-56 bg-secondary rounded-full top-75 right-2/12 z-10"></div>
        </div>
      </div>
    }
    {
